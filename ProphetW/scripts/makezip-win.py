...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\ProphetW Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\ProphetW Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\ProphetW manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\ProphetW.vst3\\Contents\\x86_64-win\\ProphetW.vst3",
      projectpath + "\\build-win\\ProphetW_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\ProphetW-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\ProphetW-app_x64.pdb"  
  ]
